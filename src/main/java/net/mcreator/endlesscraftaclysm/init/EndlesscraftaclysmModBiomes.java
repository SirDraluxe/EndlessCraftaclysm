...
@EventBusSubscriber
public class EndlesscraftaclysmModBiomes {
		...
		for (LevelStem levelStem : levelStemTypeRegistry.stream().toList()) {
			...
			if (dimensionType.is(BuiltinDimensionTypes.NETHER) || dimensionType.is(BuiltinDimensionTypes.OVERWORLD)) {
				if (levelStem.generator() instanceof NoiseBasedChunkGenerator noiseGenerator) {
					((EndlesscraftaclysmModNoiseGeneratorSettings) (Object) noiseGenerator.generatorSettings().value()).setendlesscraftaclysmDimensionTypeReference(dimensionType);
				}
			}
		}
	}
	...
	public static <T> Climate.ParameterList<T> adaptPresetParameterList(Identifier idArg, Climate.ParameterList<T> originalList, Function<ResourceKey<Biome>, T> lookup) {
		...
		if (idArg.equals(OVERWORLD_BIOMESOURCE_PRESET_ID))
			return EndlesscraftaclysmModBiomes.modifyOverworldParameterPoints(originalList, lookup);
		return originalList;
	}
	...
	private static SurfaceRules.RuleSource injectOverworldSurfaceRules(SurfaceRules.RuleSource currentRuleSource) {
		...
		customSurfaceRules.add(preliminarySurfaceRule(ResourceKey.create(Registries.BIOME, Identifier.fromNamespaceAndPath("endlesscraftaclysm", "nexurian_mainland")), EndlesscraftaclysmModBlocks.NEXURIAN_SOIL.get().defaultBlockState(),
				EndlesscraftaclysmModBlocks.NEXURIAN_STONE.get().defaultBlockState(), EndlesscraftaclysmModBlocks.NEXURIAN_SILT.get().defaultBlockState()));
	...
	public static <T> Climate.ParameterList<T> modifyOverworldParameterPoints(Climate.ParameterList<T> originalList, Function<ResourceKey<Biome>, T> lookup) {
		...
		parameters.add(new Pair<>(new Climate.ParameterPoint(Climate.Parameter.span(-0.75f, 0.25f), Climate.Parameter.span(0.35f, 0.5f), Climate.Parameter.span(0.02f, 0.84f), Climate.Parameter.span(-0.83f, -0.05f), Climate.Parameter.point(0.0f),
				Climate.Parameter.span(-1f, 1f), 0), lookup.apply(ResourceKey.create(Registries.BIOME, Identifier.fromNamespaceAndPath("endlesscraftaclysm", "nexurian_mainland")))));
		parameters.add(new Pair<>(new Climate.ParameterPoint(Climate.Parameter.span(-0.75f, 0.25f), Climate.Parameter.span(0.35f, 0.5f), Climate.Parameter.span(0.02f, 0.84f), Climate.Parameter.span(-0.83f, -0.05f), Climate.Parameter.point(1.0f),
				Climate.Parameter.span(-1f, 1f), 0), lookup.apply(ResourceKey.create(Registries.BIOME, Identifier.fromNamespaceAndPath("endlesscraftaclysm", "nexurian_mainland")))));
	...
	public interface EndlesscraftaclysmModNoiseGeneratorSettings {
		void setendlesscraftaclysmDimensionTypeReference(Holder<DimensionType> dimensionType);
	}
}